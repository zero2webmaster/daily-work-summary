...
3. Repositories are sorted by activity (most commits first)
4. **Optional AI** generates 3-5 accomplishment bullets per active repo (fallback uses commit message summaries)
...
When an AI provider key is configured, each repo's commit messages are sent to your chosen model with a prompt like:

> *"Produce 3-5 concise accomplishment bullets from these commit messages. Keep a conversational but professional tone. Do not invent work."*

**Without AI** — you get fallback bullets based on commit messages:
...
**With AI** — each repo gets 3-5 accomplishment bullets:

```
**my-website** (3 commits)
• Integrated caching and translation workflow improvements for DeepL content
• Resolved SEO metadata issues affecting homepage indexing
• Refactored queue processing for cleaner email delivery handling
```
...
*Version: 1.5.0 | Last Updated: 2026-04-14*
