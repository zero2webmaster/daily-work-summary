...
4. Each repo gets **3-5 conversational bullets** covering features, refactors, bug fixes, and outcomes
5. The result is saved as `summaries/YYYY-MM-DD-GitHub-Daily-Summary.md` and emailed as HTML with subject `Daily Cursor Work - [DATE]`

---

## How AI Repo Bullets Work

When an AI provider key is configured, each repo's commit messages are sent to your selected model to generate **3-5 concise bullet lines**.

Prompt behavior:
- Focuses on outcomes (features, refactors, bug fixes, accomplishments)
- Avoids commit-by-commit enumeration and SHA/count references
- Keeps bullets concise and conversational

If no AI key is set, the script falls back to deterministic bullet generation from commit messages, so output still matches the 3-5 bullet format.

Example output:

```
**zero2webmaster/z2w-multi-lingual-plugin** (7 commits)
• Shipped DeepL translation caching and reduced repeated API calls
• Fixed hreflang handling to improve multilingual SEO coverage
• Refined queue prioritization so analytics-critical pages process first
```
...
*Version: 1.5.0 | Last Updated: 2026-03-12*
