...
- Build one section per active repo, sorted globally by commit count (most active first)
- Repo header format: `**owner/repo-name**`
- Write 3–5 conversational bullets per repo that summarize features, fixes, refactors, and accomplishments
- Optional AI summary: if a provider key is configured, include its one-sentence thematic summary as part of the conversational bullets
- Truncate commit messages to 80 characters before bullet synthesis
- If zero commits across all repos: `No work today – hope you enjoyed the rest!`

### Step 4: Save Markdown Archive
- Write markdown to repository root as `YYYY-MM-DD-GitHub-Daily-Summary.md`
- Also write HTML render to `.tmp/YYYY-MM-DD-GitHub-Daily-Summary.html` for email body delivery
- Workflow commits the markdown archive file to git
...
- Summary archive files in repo root provide historical record
