...
- **Timezone:** `EMAIL_TIMEZONE` variable (e.g. America/New_York) for subject date and archive filename date. Default: America/New_York.
...
- Fetch ALL repos in `zero2webmaster` org (or orgs listed in `TARGET_GITHUB_ORGS`, comma-separated)
...
- Generate **3-5 conversational accomplishment bullets per repo** (AI when configured, deterministic fallback otherwise)
...
- Write separate HTML body file for email delivery
