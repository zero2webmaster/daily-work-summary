...
- Scan repos from both personal account and org memberships (including zero2webmaster)
- Sort repositories globally by commit count (most active first)
- Output format per repo:
  ...
  **Repo Name**
  ...
  • Bullet 3
  ```
- Target 3-5 bullets per repo in a conversational but professional tone
- If AI provider keys are available, use AI to synthesize accomplishment bullets from commit messages
- If AI is unavailable, fall back to clean commit-derived bullets (de-duplicated, truncated)
...
- `generate_summary.py` outputs `send_email=true/false` plus markdown/html file paths to `$GITHUB_OUTPUT`; workflow email step is conditional on that value
