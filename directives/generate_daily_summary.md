...
- Group commits by repository, across all zero2webmaster org + personal repos
- Sort all repos globally by commit count (most active first)
- Repo header: `## repo-name (N commits)`
- Include full owner/repo in italics below the repo header
...
- Format each repo section:
  ...
  * commit message 2
  ```
- Show up to 5 commit-message bullets per repo, plus an overflow bullet when there are more
...
- Write a temporary HTML companion at `.tmp/daily-summary-YYYY-MM-DD.html` for email rendering
