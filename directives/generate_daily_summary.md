...
- Sort all active repos globally by commit count (most active first)
- For each repo, produce conversational, structured output:
  - Header: `**owner/repo-name** (N commits)`
  - 3-5 bullets summarizing features, refactors, fixes, and accomplishments
- Optional AI summaries:
  - One-sentence theme summary per repo (for Airtable/webhook enrichment)
  - 3-5 AI bullets per repo when provider key is available
  - Providers: `openrouter`, `anthropic`, `gemini`, `openai`
- If no commits across all repos: `No work today – hope you enjoyed the rest!`
- Truncate raw commit lines to 80 chars when used as fallback inputs

### Step 4: Save Markdown Archive
- Write markdown archive to `YYYY-MM-DD-GitHub-Daily-Summary.md` (repo root)
- Write separate email HTML body to `.tmp/YYYY-MM-DD-GitHub-Daily-Summary-email.html`
