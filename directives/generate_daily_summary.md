...
- Build a project-by-project digest (no raw commit dump)
- Repo header uses full name: `## owner/repo`
- Sort repos by commit count (most active first)
- Generate 3-5 conversational bullets per repo summarizing accomplishments:
  - features shipped
  - bug fixes
  - refactors/performance/testing/docs work
- Optional AI summary: If any AI key set (OPENROUTER_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY), generate a one-sentence theme and use it as a leading repo bullet. Use AI_PROVIDER variable to choose: openrouter, anthropic, gemini, openai. Auto-detects from first available key if unset.
- Truncate individual commit messages to 80 characters when composing bullets
