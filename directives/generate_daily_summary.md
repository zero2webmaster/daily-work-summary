...
- Format each repo section as:
  ...
  **repo-name** (N commits)
  ...
  ...
  ```
- Generate **3-5 bullets per repo** in a conversational, professional tone
- Optional AI summary bullets: if AI provider key is available, generate bullets from commit themes
- Fallback summary bullets: if AI is unavailable, derive bullets heuristically from commit messages (feature/fix/refactor/tests/docs/ops buckets)
- Truncate long commit messages used for fallback bullet generation
- If zero commits across all repos: "No work today – hope you enjoyed the rest!"

### Step 4: Save Daily Archive Files
- Write markdown archive to `YYYY-MM-DD-GitHub-Daily-Summary.md` (repo root)
- Write email body HTML to `YYYY-MM-DD-GitHub-Daily-Summary.html` (repo root)
...
- Body: HTML file generated by `generate_summary.py`
- `generate_summary.py` outputs `has_summary`, `send_email`, and `summary_html_file` to `$GITHUB_OUTPUT`; workflow email step is conditional on those values
...
| Email Body File | `YYYY-MM-DD-GitHub-Daily-Summary.html` | HTML (always) |
...
- Summary archive files in repo root provide historical record (`YYYY-MM-DD-GitHub-Daily-Summary.md`)
