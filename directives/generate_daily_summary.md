...
- Sort repos within each owner by commit count (most active first)
...
- Deterministic fallback: without AI, use up to 5 unique truncated commit subjects as bullets.
- Format each repo section:
  ```
  ### repo-name
  *N commits in the last 24 hours*
  * Summary bullet 1
  * Summary bullet 2
  ...
  ```
- Show 3-5 bullets when AI is configured; deterministic fallback shows up to 5 commit-subject bullets.
- Truncate individual fallback commit messages to 80 characters
...
- Write HTML email body to `.tmp/daily-summary-email.html`
