...
- Flatten active repositories into a single list and sort globally by commit count (most active first)
- Repo header format: `**Repo Name**` (repo name only)
- Generate **3-5 conversational bullets per repo** that summarize features, fixes, refactors, and accomplishments from the last 24 hours
- Optional AI summary: If any AI key set (OPENROUTER_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY), generate one-sentence theme per repo. Use AI_PROVIDER variable to choose: openrouter, anthropic, gemini, openai. Auto-detects from first available key if unset.
- Normalize commit subjects (remove conventional commit prefixes) and bucket by category to build concise bullets
- Truncate individual commit message subjects to 80 characters before summarization
...
- Write HTML email body to `.tmp/YYYY-MM-DD-GitHub-Daily-Summary.email.html`
