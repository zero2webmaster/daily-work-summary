...
Generate a smart daily summary of all GitHub commits across every zero2webmaster and personal repository from the last 24 hours. Deliver via email, Airtable, Slack, and/or Discord, and save a Markdown archive in the repository root.
...
- Build a global list of active repos (personal + organization) and sort by commit count (most active first)
...
- Format each repo section:
  ```
  **owner/repo**
  • 3-5 conversational bullets summarizing features, refactors, bug fixes, accomplishments
  ```
- Bullets are synthesized from commit themes (not a raw per-commit dump)
- Truncate individual commit messages to 80 characters before synthesis
- If zero commits across all repos: "No work today – hope you enjoyed the rest!"

### Step 4: Save Markdown Archive
- Write to `YYYY-MM-DD-GitHub-Daily-Summary.md` in repository root
...
- Body: HTML-formatted summary from `.tmp/daily-summary-email-YYYY-MM-DD.html`
...
- Root archive files (`YYYY-MM-DD-GitHub-Daily-Summary.md`) provide historical record
