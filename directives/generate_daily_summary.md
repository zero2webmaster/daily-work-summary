...
Generate a conversational daily summary of all GitHub commits across every zero2webmaster and personal repository from the last 24 hours. Deliver via email, Airtable, Slack, and/or Discord, and save a Markdown archive.
...
- Fetch all personal repos owned by the authenticated user (including private)
- Fetch all repos in the `zero2webmaster` organization (including private repos visible to the PAT)
...
- Skip repos with zero commits in the window

### Step 3: Generate Smart Summary
- Build one section per active repository, sorted by commit count (most active first)
- Repo header format: `**owner/repo-name**`
- Generate 3-5 conversational bullet points per repo summarizing accomplishments (feature/fix/refactor/ops/docs themes inferred from commit text)
- Optional AI summary: if any AI key is set (OPENROUTER_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY), prepend a high-level “Big picture” bullet for each repo
- Keep commit details concise (messages truncated to 80 characters)
...
- Subject: `Daily Cursor Work - [DATE]` (workflow date string in configured timezone)
- Body: HTML-formatted summary rendered from Markdown
...
| Non-target owners | Ignore repos not owned by user or `zero2webmaster` |
