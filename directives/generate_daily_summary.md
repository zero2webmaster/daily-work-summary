...
Generate a smart daily summary of all GitHub commits across every zero2webmaster and personal repository from the last 24 hours. Deliver via email, Airtable, Slack, and/or Discord, and save a Markdown archive with user-facing naming.
...
- Fallback for local automations: use `gh auth token` when `PAT_GITHUB` is missing
- Fetch ALL personal repos owned by user (`type="owner"`)
- Fetch org repos from `zero2webmaster` + any org memberships
- Deduplicate by `owner/repo` full name
...
- Build a single global repo list (no owner grouping), sorted by commit count descending
- Repo header: `**owner/repo** (N commits)`
...
- Format each repo section:
  ...
  **owner/repo** (N commits)
  _AI summary sentence_ (optional)
  • Feature work: ...
  • Bug fixes: ...
  • Refactors: ...
  ```
- 3-5 conversational bullets per active repo (or fewer if <3 commits)
- Truncate individual commit messages to 80 chars when needed
- If zero commits across all repos: "No work today – hope you enjoyed the rest!"

### Step 4: Save Markdown Archive
- Write Markdown archive to `YYYY-MM-DD-GitHub-Daily-Summary.md` in repo root
- Write HTML companion to `summaries/daily-summary-YYYY-MM-DD.html` for email body rendering
