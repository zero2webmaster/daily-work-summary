...
- Use repo header: `**owner/repo-name**`
- Generate 3-5 conversational bullets per repo summarizing accomplishments (features, fixes, refactors, outcomes)
- Optional AI summary: If any AI key set (OPENROUTER_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY), prepend one thematic sentence per repo. Use AI_PROVIDER variable to choose: openrouter, anthropic, gemini, openai. Auto-detects from first available key if unset.
- Format each repo section:
  ```
  **owner/repo-name**
  • [Optional AI theme sentence]
  • [Accomplishment bullet 1]
  • [Accomplishment bullet 2]
  • [Accomplishment bullet 3]
  ```
- Commit messages are still normalized/truncated during synthesis for concise bullets
- If zero commits across all repos: "No work today – hope you enjoyed the rest!"

### Step 4: Save Markdown Archive
- Write to `YYYY-MM-DD-GitHub-Daily-Summary.md` (repo root)
